 ...
 * @typedef {import("../types").UserModel} User
 */
	...
	await registerUser
		...
		.catch((e) => {
			res.status(400).json({
				...
				error:
					e.code == 11000
						? `El registro [${Object.keys(e)
								.map((key) =>
									key == "keyValue"
										? Object.keys(e[key]) + " : " + Object.values(e[key])
										: null
								)
								.join("")}] ya existe en la base de datos`
						: e.toString(),
			});
		});
}
...
export async function registerAdmin(req, res) {
	...
	if (adminKey === ADMIN_KEY) {
		...
		res.status(400).json({
			status: "fail",
			error: "La llave de creacion de administrador no es valida",
		});
	}
}
...
export async function updateUser(req, res) {
	...
	await Usuario.updateOne(
		{ _id: id },
		{ email, accessEmpresas, accessInventarios, accessProductos },
		{},
		(err, result) => {
			if (err) {
...
/**
 * @type {RouterFunction}
 * @param {RouteRequest & {user: User}} req
 * @param {*} res
 */
export async function updatePassword(req, res) {
	...
	if (isValid) {
		const { salt, hash } = passwordGenerator(password);
		// const result =
		await Usuario.updateOne({ _id: id }, { salt, hash }, {}, (err, result) => {
			if (err) {
				return res.status(400).json({ status: "error", message: err });
			}
			if (result.n != result.nModified) {
		...
		res.status(400).json({
			status: "error",
			message:
				"La contraseña actual no coincide con las credenciales almacenadas ✖",
		});
	}
}
