...
const App = () => {
	...
	const session = async () => {
		await fetch("/uac/user")
			...
			.then((data) => {
				...
				dispatch({
					type: data ? type.LOG_IN : type.LOG_OUT,
					payload: data
						? {
								username,
								email,
	...
	if (!isLoading)
		return (
			<>
				<div className={`pageloader is-info ${isLoading && "is-active"}`}>
					<span className="title">Cargando app...</span>
				</div>
				<Navbar />
				<Switch>
					<Route path="/clientes">
						{!state.userData ? (
							<Redirect to="/login" />
						) : state.userData.isAdmin || state.userData.accessEmpresas[0] ? (
							<JsonCliente accessEmpresas={state.userData.accessEmpresas} />
						) : (
					...
					<Route path="/productos">
						{!state.userData ? (
							<Redirect to="/login" />
						) : state.userData.isAdmin || state.userData.accessProductos[0] ? (
							<ProductosUI accessProductos={state.userData.accessProductos} />
						) : (
							<UnauthorizedComponent />
						)}
					</Route>
					<Route path="/inventarios">
						{!state.userData ? (
						  ...
						  state.userData.accessInventarios[0] ? (
							<Inventory accessInventarios={state.userData.accessInventarios} />
						) : (
							<UnauthorizedComponent />
						)}
					</Route>
