	...
	const logOutUser = async () => {
		...
		const result = await fetch("/uac/logout")
			.then((res) => {
				...
				else throw "Ha ocurrido un error al cerrar sesion";
			})
			.catch((e) => {
				console.error(e);
				return;
		...
		<>
			<div className="navbar-item has-dropdown is-hoverable">
				...
				<div className="navbar-dropdown">
					{isAdmin && (
						<Link className="navbar-item" to="/admin/usuarios">
							👥 Gestión de usuarios
						</Link>
					)}
					{isAdmin && (
						<Link className="navbar-item" to="/admin/ptype-mgmt">
	...
	return (
		<>
			<Link className="" to="/login">
				<div className="navbar-item ">
					<span className="button is-link">Ingresar</span>
				</div>
			</Link>
